
export interface ClientData {
  src: string ;
  alt: string;
  className?: string;
}

export interface ChooseCardProsData {
  title: string;
  subtitle: string;
  Icon: React.ReactNode;
}

export const ClientImages: ClientData[] = [
  {
    src: '/images/ALLURE-NAINITAL-LOGO.png',
    alt: "Client1",
    className: "#000000",
  },
  {
    src: '/images/Client2.png',
    alt: "Client2",
    className: "#ffffff",
  },
  {
    src: '/images/Client3.png',
    alt: "Client3",
  },
  {
    src: '/images/BG-logo.jpg',
    alt: "Client4",
  },
  {
    src: '/images/BURKEY-BLACK-LOGO-01.png',
    alt: "Client5",
  },
  {
    src: '/images/Client6.png',
    alt: "Client6",
  },
  // {
  //   src: '/images/D&G-logo-03-1.png,
  //   alt: "Client7",
  // },
  {
    src: '/images/DOUBLE-TREE-LOGO-01.png',
    alt: "Client8",
  },
  {
    src: '/images/Dunagiri-Logo-Square.png',
    alt: "Client9",
  },
  // {
  //   src: '/images/HUMRAHI-TRAVELS-LOGO.jpg',
  //   alt: "Client10",
  // },
  {
    src: '/images/Hyatt-regency-logo.jpg',
    alt: "Client11",
  },
  {
    src: '/images/JW-Mariott-logo.png',
    alt: "Client12",
  },
  {
    src: '/images/Novotel-Logo.png',
    alt: "Client13",
  },
  {
    src: '/images/Radisson-Logo.png',
    alt: "Client14",
  },
  {
    src: '/images/Ramada-Logo.jpg',
    alt: "Client15",
  },
  {
    src: '/images/Taj-hotel-logo.png',
    alt: "Client16",
  },
  // {
  //   src: '/images/The_Park_Hotels.jpg',
  //   alt: "Client17",
  // },
  {
    src: '/images/peace@peak.jpg',
    alt: "Client18",
  },
  {
    src: '/images/logo_uv-removebg-preview-e1701685619775.webp',
    alt: "Client19",
  },
  {
    src: '/images/bunker-house.jpg',
    alt: "Client20",
  },
  {
    src: '/images/Red-Rock-villa.jpg',
    alt: "Client21",
  },
  {
    src: '/images/wabi-sabi.jpg',
    alt: "Client22",
  },
  {
    src: '/images/ebc.jpg',
    alt: "Client23",
  },
  {
    src: '/images/Riviera.jpg',
    alt: "Client24",
  },
  {
    src: '/images/footerspicelogo.png',
    alt: "Client25",
    className: "#21413e",
  },
  {
    src: '/images/chefkenzo.png',
    alt: "Client26",
  },
  {
    src: '/images/eracamp.png',
    alt: "Client27",
  },
  {
    src: '/images/Grandview_resort.jpg',
    alt: "Client28",
  },
  {
    src: '/clientsLogo/468F.png',
    alt: "Client29",
  },
  {
    src: '/clientsLogo/Aaoji.png',
    alt: "Client30",
    className: "#000000",
  },
  {
    src: '/clientsLogo/STERNWHEELER.jpg',
    alt: "Client31",
  },
  {
    src: '/clientsLogo/Season-suites.png',
    alt: "Client32",
  },
  {
    src: '/clientsLogo/emeraldtrail.png',
    alt: "Client33",
  },
  {
    src: '/clientsLogo/kiyostay.png',
    alt: "Client34",
  },
  {
    src: '/clientsLogo/lakehill.png',
    alt: "Client35",
    
  },
  {
    src: '/clientsLogo/maati.png',
    alt: "Client36",
    className: "#000000",
  },
  {
    src: '/clientsLogo/sprav.jpg',
    alt: "Client37",
    className: "#000000",
  },
  {
    src: '/clientsLogo/Park Cafe.png',
    alt: "Client38",
    className: "#ffffff",
  },
  {
    src: '/clientsLogo/deuropa.png',
    alt: "Client39",
  },
  {
    src: '/clientsLogo/dhruvees.png',
    alt: "Client40",
    className: "#000000",
  },
  {
    src: '/clientsLogo/dunagiri-retreat.png',
    alt: "Client41",
  },
  {
    src: '/clientsLogo/hilton.png',
    alt: "Client42",
  },
  {
    src: '/clientsLogo/hotel-platinum-shravasti.png',
    alt: "Client43",
  },
  {
    src: '/clientsLogo/mahabir-palace.png',
    alt: "Client44",
    className: "#fff",
  },
  {
    src: '/clientsLogo/oak-climbing.png',
    alt: "Client45",
  },
  {
    src: '/clientsLogo/peace@peak.png',
    alt: "Client46",
  },
  {
    src: '/clientsLogo/tents&trails.png',
    alt: "Client47",
  },
  {
    src: '/clientsLogo/minimalist.png',
    alt: "Client48",
    className: "#ffffffbe",
  },
  {
    src: "/clientsLogo/magnolia.png",
    alt: "Client49",
    className: "#000000",
  },
  {
    src: "/clientsLogo/white-ridge.png",
    alt: "Client50",
  },
  {
    src: "/clientsLogo/avr.jpg",
    alt: "Client51",
    className: "#ffffff",
  },
  {
    src: "/clientsLogo/tulipInn.png",
    alt: "Client52",
  },
  {
    src: "/clientsLogo/Hotel-Green-Castle.jpg",
    alt: "Client53",
  },
  {
    src: "/clientsLogo/aqua-dunhinda.jpg",
    alt: "Client54",
  },
  {
    src: "/clientsLogo/shreeiyengar.png",
    alt: "Client55",
  },
  {
    src: "/clientsLogo/kamalfarms.png",
    alt: "Client56",
  },
  {
    src: "/clientsLogo/petal-panyaram.png",
    alt: "Client57",
  },
  {
    src: "/clientsLogo/7Mile.png",
    alt: "Client58",
  },
  {
    src: "/clientsLogo/The-Jharna.png",
    alt: "Client59",
  },
  {
    src: "/clientsLogo/chocolate-room.png",
    alt: "Client60",
  },
  {
    src: "/clientsLogo/kumaon-bliss.jpg",
    alt: "Client61",
  },
  {
    src: "/clientsLogo/white-ridge.png",
    alt: "Client62",
  },
  {
    src: "/clientsLogo/tailgaters.png",
    alt: "Client63",
  },
  {
    src: "/clientsLogo/outer-ring.png",
    alt: "Client64",
  },
  {
    src: "/clientsLogo/luxuryvibes.jpg",
    alt: "Client65",
  }
];
